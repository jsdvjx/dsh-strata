...
悬停在**用户色块或它的锚点**上，会展开一摞层叠卡片——**这个会话里你说过的每一句话，
包括还没加载进窗口的**（读取会话自己的导出日志）。悬停的那张展开成多行预览，其余压成带
时间戳的单行薄片；未加载的显示空心圆点和"未加载"标记。**滚轮**在牌堆里换焦，**点击**
跳到那条消息——未加载的会先自动连锁载入缺失的历史，再精准落位。
