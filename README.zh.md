...
**点击用户锚点圆点**打开线索墙：全屏铺开的发言板——**这个会话里你说过的每一句话，
包括还没加载进窗口的**（读取会话自己的导出日志）。卡片按时间序排成响应式网格，
带加粗编号、时间戳和多行预览；未加载的用虚线框加"未加载"标记，你进来时对应的那条
高亮并滚动到视野中央。**点击卡片**跳到那条消息——未加载的先自动连锁载入缺失历史再落位。
**Esc**、点击空白背景或 ✕ 关闭。悬停圆点仍是轻量的单条预览。
